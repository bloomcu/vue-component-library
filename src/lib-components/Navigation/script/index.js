...
export default function SubNavigationScript() {
  // Usage: codyhouse.co/license
  (function () {
    const SideNav = function (element) {
    ...
    function getFirstFocusable(sidenav) {
      ...
      const focusableEle = sidenav.navList[0].querySelectorAll(
    ...
    function checkSideNavLayout(sidenav) {
      ...
      const layout = getComputedStyle(sidenav.element, ':before')
    ...
    const sideNav = document.getElementsByClassName('js-subnav');
    const SideNavArray = [];
    let j = 0;
    if (sideNav.length > 0) {
      ...
      if (j > 0) {
        ...
        const customEvent = new CustomEvent('update-sidenav');
        ...
        function doneResizing() {
          for (let i = 0; i < SideNavArray.length; i++) {
            (function (i) {
